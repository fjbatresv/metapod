...
use Metapod\SoporteBundle\Model\BitacoraQuery;
 ...
 * @method BitacoraQuery orderByUsuarioId($order = Criteria::ASC) Order by the usuario_id column
 ...
 * @method BitacoraQuery groupByUsuarioId() Group by the usuario_id column
 ...
 *
 * @method BitacoraQuery leftJoinUsuario($relationAlias = null) Adds a LEFT JOIN clause to the query using the Usuario relation
 * @method BitacoraQuery rightJoinUsuario($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Usuario relation
 * @method BitacoraQuery innerJoinUsuario($relationAlias = null) Adds a INNER JOIN clause to the query using the Usuario relation
 *
 ...
 * @method Bitacora findOneByUsuarioId(int $usuario_id) Return the first Bitacora filtered by the usuario_id column
 ...
 * @method array findByUsuarioId(int $usuario_id) Return Bitacora objects filtered by the usuario_id column
...
abstract class BaseBitacoraQuery extends ModelCriteria
{
    ...
    public function __construct($dbName = null, $modelName = null, $modelAlias = null)
    {
        ...
        if (null === $modelName) {
            $modelName = 'Metapod\\SoporteBundle\\Model\\Bitacora';
        }
    ...
    {
        $sql = 'SELECT `id`, `descripcion`, `direccion`, `fecha_hora`, `usuario_id`, `tabla`, `dato_tabla`, `error`, `dato_error`, `estado`, `created_by`, `updated_by`, `created_at`, `updated_at` FROM `bitacora` WHERE `id` = :p0';
        try {
    ...
    public function filterByUsuarioId($usuarioId = null, $comparison = null)
    {
        if (is_array($usuarioId)) {
            ...
            if (isset($usuarioId['min'])) {
                $this->addUsingAlias(BitacoraPeer::USUARIO_ID, $usuarioId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($usuarioId['max'])) {
                $this->addUsingAlias(BitacoraPeer::USUARIO_ID, $usuarioId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(BitacoraPeer::USUARIO_ID, $usuarioId, $comparison);
    }
    ...
    public function filterByUsuario($usuario, $comparison = null)
    {
        if ($usuario instanceof Usuario) {
            return $this
                ->addUsingAlias(BitacoraPeer::USUARIO_ID, $usuario->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(BitacoraPeer::USUARIO_ID, $usuario->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Metapod\SoporteBundle\Model\UsuarioQuery A secondary query class using the current class as primary query
     */
    public function useUsuarioQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinUsuario($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Usuario', '\Metapod\SoporteBundle\Model\UsuarioQuery');
    }
