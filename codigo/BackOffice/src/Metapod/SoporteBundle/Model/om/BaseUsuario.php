...
abstract class BaseUsuario extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'Metapod\\SoporteBundle\\Model\\UsuarioPeer';
    ...
    /**
     * The value for the conectado field.
    ...
    /**
     * The value for the ultima_ip field.
     * @var        string
     */
    protected $ultima_ip;
    ...
    /**
     * Get the [conectado] column value.
     *
     * @return boolean
     */
    ...
    /**
     * Get the [ultima_ip] column value.
     *
     * @return string
     */
    ...
    /**
     * Sets the value of the [conectado] column.
    ...
    /**
     * Set the value of [ultima_ip] column.
     *
     * @param  string $v new value
     * @return Usuario The current object (for fluent API support)
     */
    public function setUltimaIp($v)
    {
        ...
        if ($this->ultima_ip !== $v) {
            $this->ultima_ip = $v;
            $this->modifiedColumns[] = UsuarioPeer::ULTIMA_IP;
        }
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(UsuarioPeer::CONECTADO)) {
            $modifiedColumns[':p' . $index++]  = '`conectado`';
        }
        if ($this->isColumnModified(UsuarioPeer::ULTIMA_IP)) {
            $modifiedColumns[':p' . $index++]  = '`ultima_ip`';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`avatar`':
                        ...
                        break;
                    case '`conectado`':
                        $stmt->bindValue($identifier, (int) $this->conectado, PDO::PARAM_INT);
                        break;
                    case '`ultima_ip`':
                        $stmt->bindValue($identifier, $this->ultima_ip, PDO::PARAM_STR);
                        break;
                    case '`empresa_id`':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UsuarioPeer::ULTIMA_IP)) $criteria->add(UsuarioPeer::ULTIMA_IP, $this->ultima_ip);
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setConectado($this->getConectado());
        $copyObj->setUltimaIp($this->getUltimaIp());
